...
<!-- Generieren der definitionen.xml-Datei (im xml_daten -> selbstgenerierte-Ordner), in der sämtliche Definitionen aus allen Lerneinheiten gesammelt werden -->
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template name="xsl:initial-template">
        <div class="definitions">
            ...
            <xsl:for-each select="collection('../xml_daten/wissensbereiche/?select=*.xml;recurse=yes')">
                <!-- Anwenden der Templates für sämtliche Definitionen in der jeweiligen Datei -->
    ...
    <xsl:template match="definition">
        <div class="definition">
            <!-- Ausstatten des Definition-Elements mit einem id-Attribut -> Gibt den definierten Begriff an -->
            <xsl:attribute name="id">
                <xsl:value-of select="@title"/>
            </xsl:attribute>
            <!-- Ausstatten des Definition-Elements mit einem label-Attribut -> Gibt die ursprüngliche ID aus der XML-Datei an -->
            <xsl:attribute name="label">
                <xsl:value-of select="@label"/>
            </xsl:attribute>
            <!-- Ausstatten des Definition-Elements mit einem uri-Attribut -> Gibt den Wissensbereich an -->
            ...
            <!-- Ausstatten des Definition-Elements mit einem file-Attribut -> Gibt die Lerneinheit/Datei an -->
