<!-- ANWENDUNG UND ZIEL -->
<!-- Anwenden des Stylesheets auf die definitionen.xml-Datei (im daten -> xml_selbstgenerierte-Ordner), in der sämtliche Defintionen aus allen Lerneinheiten gesammelt wurden -->
...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- AUFTEILUNG WISSENSBEREICHE/ABSCHNITTE -->
    ...
    <!-- EINZELNE DEFINITIONEN/EINTRÄGE -->
    <xsl:template match="div[@class='definition']">
        <!-- Einfügen des Glossareintrags als Listenelement -->
        <li class="gloss_entry">
            <!-- Einfügen des definierten Begriffs (-> Wird dann blau/fett dargestellt) -->
            <span class="gloss_term">
                <!-- Verlinken des definierten Begriffs mit der Website/Lerneinheit, in der diese Definition zu finden ist -->
                <a class="gloss_link">
                    <xsl:attribute name="href">
                        <xsl:text>{{ '_pages/wissensbereiche/</xsl:text>
                        <!-- Nutzen der uri- (= Wissensbereich) und der file- (= Lerneinheit/Datei) Attribute zur Generierung des korrekten Dateipfads für den Link -->
                        <xsl:value-of select="@uri"/>
                        <xsl:text>/</xsl:text>
